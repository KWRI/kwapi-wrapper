...
describe('LeadRoutingTest', () => {
    describe('testList()', () => {
        it('Should get list of IDs', (done) => {
            ...
            .then(() => KwApi.LeadRouting().lists())
            .then((result) => {
                result.per_page.should.above(0)
                result.current_page.should.be.a('number')
                done()
            })
            .catch(err => done(err))
        })
        ...
        describe('testCreateList()', () => {
            it('Could Create a List', (done) => {
                ...
                KwApi.LeadRouting().createList(data.name, data.router, data.hash).then((result) => {
                    result.hash.should.ownProperty('length')
                    result.should.have.any.keys('id')
                    result.should.have.any.keys('hash')
                    result.should.have.any.keys('name')
                    createdId = result.id
                    done()
                })
        ...
        describe('testReadList()', () => {
            ...
            it('Should Display a List', (done) => {
                ...
                .then((result) => { createdId = result.id })
        ...
        describe('testUpdateList()', () => {
            it('Could Update a List', (done) => {
                ...
                KwApi.LeadRouting().createList(data.name, data.router, data.hash)
                    ...
                    .then((result) => {
                        result.id.should.equal(createdId)
                        result.api_user_id.should.be.a('number')
                        result.name.should.be.a('string')
                        result.hash.should.be.a('string')
                        result.router.should.be.a('string')
                        createdId = result.id
                        done()
                    })
                    .then(() => KwApi.LeadRouting().removeList(createdId))
        ...
        describe('testCreateAgent()', () => {
            it('Could Create an Agent on specific list', (done) => {
                ...
                KwApi.LeadRouting().createList(data.name, data.router, data.hash)
                    ...
                    .then((result) => {
                        result.kw_uid.should.be.a('string')
                        result.first_name.should.be.a('string')
                        result.last_name.should.be.a('string')
            ...
            it('Could Bulk create multiple Agents on specific list', (done) => {
                ...
                KwApi.LeadRouting().createList(data.name, data.router, data.hash)
                    ...
                    .then((result) => {
                        ...
                        result[0].email.should.have.string('@')
                        result[0].first_name.should.be.a('string')
                        result[0].last_name.should.be.a('string')
                        result[0].active.should.equal('true')
                        result[0].kw_uid.should.be.a('string')
                    })
                    .then(() => KwApi.LeadRouting().removeList(createdId))
                    .then(() => { done() })
                    .catch(err => done(err))
